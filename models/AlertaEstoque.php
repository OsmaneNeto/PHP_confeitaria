...
class AlertaEstoque {
    ...
    public function verificarAlertasEstoque() {
        $query = "SELECT id_insumo, nome_insumo, quantidade_estoque, estoque_minimo 
        ...
        while($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            ...
            if(!$this->existeAlertaAtivo($row['id_insumo'])) {
                $this->insumo_id = $row['id_insumo'];
                $this->tipo_alerta = ($row['quantidade_estoque'] <= 0) ? 'estoque_zerado' : 'estoque_minimo';
                $this->quantidade_atual = $row['quantidade_estoque'];
    ...
    public function listarAlertasNaoVisualizados() {
        $query = "SELECT a.*, i.nome_insumo, i.unidade_medida 
                  FROM " . $this->table_name . " a
                  INNER JOIN " . $this->insumos_table . " i ON a.insumo_id = i.id_insumo
    ...
    public function obterInsumosCriticos($percentual_minimo = 0.1) {
        $query = "SELECT 
                    ...
                    i.unidade_medida,
                    CASE 
                        WHEN i.quantidade_estoque <= 0 THEN 'ZERADO'
                        WHEN i.quantidade_estoque <= i.estoque_minimo THEN 'CRÍTICO'
                        WHEN i.quantidade_estoque <= (i.estoque_minimo * 1.5) THEN 'BAIXO'
                        ELSE 'NORMAL'
                    END as status_estoque
                  FROM " . $this->insumos_table . " i
                  WHERE i.quantidade_estoque <= (i.estoque_minimo * (1 + :percentual_minimo))
                  ORDER BY (i.quantidade_estoque / i.estoque_minimo) ASC";
